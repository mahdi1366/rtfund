...
function SelectAllLetter(){
	...
	$query = "select s.*,l.*, 
		...
		where s.ToPersonID=? AND ResponseTimeout is not null ";
	$param = array( $_SESSION["USER"]["PersonID"]);
	
	if(!empty($_REQUEST["StartDate"]))
	{
		$query .= " AND ResponseTimeout >= ?";
...
$dg = new sadaf_datagrid("dg", $js_prefix_address . "ResponseReport.php?task=SelectAllLetter", "grid_div");

$dg->addColumn("", "LetterID", "", true);
$dg->addColumn("", "SendComment", "", true);

$col = $dg->addColumn("<img src=/office/icons/LetterType.gif>", "LetterType", "");
$col->renderer = "ResponseReport.LetterTypeRender";
$col->width = 30;
...
$col = $dg->addColumn("مهلت پاسخ", "ResponseTimeout", GridColumn::ColumnType_date);
$col->width = 80;

$dg->emptyTextOfHiddenColumns = true;
$dg->height = 380;
$dg->width = 800;
$dg->title = "نامه های دارای مهلت پاسخ";
...
function ResponseReport(){
	...
	this.SearchPanel.getEl().addKeyListener(Ext.EventObject.ENTER, function(keynumber,e){
		...
		ResponseReportObject.searching();
		e.preventDefault();
		e.stopEvent();
		return false;
	});
...
ResponseReport.LetterTypeRender = function(v,p,r){
...
ResponseReportObject = new ResponseReport();
