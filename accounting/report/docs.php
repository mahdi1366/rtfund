...
{
	$query = "select d.*, concat(fname,' ',lname) as regPerson, sum(bsAmount) bsSum,sum(bdAmount) bdSum
			from acc_docs d
			join acc_doc_items di using(docID)
			join persons p on(regPersonID=personID)
			where d.cycleID=" . $_SESSION["CYCLE"];
	...
	$rpg->addColumn("شماره سند مرجع/عطف", "ref_docID");	
	//$rpg->addColumn("شماره سند انبار", "storeDocID");
