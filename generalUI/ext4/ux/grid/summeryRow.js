...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
		...
		for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
			...
			if (cf.summaryType) {
				for (var j = 0, jlen = rs.length; j < jlen; j++) {
					...
					//jafarkhani----------------------
					var val = (cf.renderer) ? cf.renderer(r.get(cname),'',r) : r.get(cname);
					val = val.toString().replace(/,/g,"");
					data[cname] = Ext.ux.grid.GridSummary.Calculations[cf.summaryType]
							(val, r, cname, data, j);						
					//-------------------------------------
				}
			}
	...
	renderSummary : function(o, cs, cm) {
		...
		for (var i = 0, len = cs.length; i < len; i++) {
			...
			if (cf.summaryType || cf.summaryRenderer) {
				...
				p.value = (cf.summaryRenderer) ? cf.summaryRenderer(o.data[c.name], p, o) : o.data[c.name];
			} else {
				p.value = '';
			}
			if (p.value == undefined || p.value === "") p.value = "&#160;";
			//jafarkhani----------------------
			this.values[i] = o.data[c.name];
			//--------------------------------
			buf[buf.length] = this.cellTpl.apply(p);
			
			if(this.afterRenderSummery != "")
				this.afterRenderSummery();
		}
	...
	getSummaryValuesArray : function(){
		return this.values;
	}
});
