...
Ext.ux.Printer.BaseRenderer = Ext.extend(Object, {
   ...
   */
  print: function(component) {
    var name = component && component.xtype
             ? Ext.String.format("print_{0}_{1}", component.xtype, component.id)
             : "print";
  ...
  stylesheetPath: '/generalUI/ext4/ux/Printer/print.css'
});
...
Ext.ux.Printer.TreePanelRenderer = Ext.extend(Ext.ux.Printer.BaseRenderer, {
	...
	PrefixImgAddress : '/generalUI/ext4/resources/themes/rtl/',
		
	generateBody: function(tree) {
	
		var headings = this.headerTpl.apply({header : this.getTitle(tree)});
		var body     = [
			'<tpl for=".">',
				'<tr>',
					'<td >',
						'<tpl for="values.depthArr">',
							'<tpl if=".">',
								'&nbsp;&nbsp;&nbsp;',
							'<tpl else>',
								'<img style="vertical-align:middle;" src=' + this.PrefixImgAddress + 'elbow-line-rtl.gif >',
							'</tpl>',
						'</tpl>',
						'<tpl if="!values.isLeaf">',
							'<tpl if="values.isLast">',
								'<img style="vertical-align:middle;" src=' + this.PrefixImgAddress + 'elbow-end-minus-rtl.gif >',
							'</tpl>',
							'<tpl if="!values.isLast">',
								'<img style="vertical-align:middle;" src=' + this.PrefixImgAddress + 'elbow-minus-rtl.gif >',
							'</tpl>',
							
						'</tpl>',
						'<tpl if="values.isLeaf">',
							'<tpl if="values.isLast">',
								'<img style="vertical-align:middle;" src=' + this.PrefixImgAddress + 'elbow-end-rtl.gif >',
							'</tpl>',
							'<tpl if="!values.isLast">',
								'<img style="vertical-align:middle;" src=' + this.PrefixImgAddress + 'elbow-rtl.gif >',
							'</tpl>',
						'</tpl>',
						'{[values.text]}',							
					'</td>',
		...
		return Ext.String.format('<table id=treeTable>{0}<tpl for=".">{1}</tpl></table>', headings, body);
	},
  ...
  prepareData: function(tree) {
    ...
    var f = function(node) {
      ...
      var row = node.data;
      row["depth"] = node.getDepth();
	  
	  row["depthArr"] = new Array();
	  var parent = node.parentNode;
	  for(i=0; i<node.getDepth(); i++)
	  {
		row["depthArr"].push(parent.isLast());
		parent = parent.parentNode;
	  }
	  row["depthArr"].reverse();
	  
	  row["isLeaf"] = node.isLeaf();
	  row["isLast"] = node.isLast();
      //the property used in the first column is renamed to 'text' in node.attributes, so reassign it here
      //row["text"] = node.data.text;      
      data.push(row);
    };
