...
class DROPDOWN extends common_component
{
	...
	function bind_dropdown($selectedID = "")
	{
		...
		$str = "<select class='x-form-text x-form-field' 
					name='" . $this->id . "'
					id='" . $this->id . "'
					style='" . $this->Style . "' " . $this->Event . ">";
...
class AutoComplete_DROPDOWN extends DROPDOWN
{
	...
	function bind_dropdown($selectedID = "")
	{
		$return = parent::bind_dropdown($selectedID);
		
		$return .= "<script>Ext.onReady(function(){
			new Ext.form.ComboBox({
			id:'ext_$this->id',
			typeAhead: true,
			hiddenName: '$this->id',
        	triggerAction: 'all',";
        $return .= (!empty($this->formName)) ? "transform:document.forms['$this->formName'].$this->id," :
        	"transform: '$this->id',";
