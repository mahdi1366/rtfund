...
CREATE VIEW person_courses AS select `r`.`PersonID` AS `PersonID`,`p`.`from_date` AS `from_date`,`p`.`to_date` AS `to_date`,
`p`.`hours` AS `total_hours`,`p`.`p_id` AS `p_id`,`p`.`group_id` AS `group_id`,`r`.`related` AS `related`,
`p`.`internal` AS `internal`,`r`.`get_award` AS `get_award`,`r`.`award_date` AS `award_date`,`r`.`get_group` AS `get_group`,
`r`.`group_date` AS `group_date`,`r`.`message` AS `comments`,`p`.`lesson_id` AS `course_id`,`p`.`group_id` AS `group_course_id`,
`p`.`deliver_id` AS `deliver_id`,`l`.`lesson_name` AS `title`,`r`.`state` AS `state`,`r`.`register_no` AS `register_no`
from ((`ease`.`SED_presentation` `p`
join `ease`.`SED_registeration` `r` on(((`p`.`p_id` = `r`.`p_id`) and (`p`.`group_id` = `r`.`group_id`))))
left join `ease`.`SED_lesson` `l` on((`l`.`lesson_id` = `p`.`lesson_id`)))
where ((`r`.`status_hrms` = _utf8'YES') and (`r`.`state` = 2));

CREATE VIEW UsersSystems AS select `framework`.`UsersSystems`.`RecID` AS `RecID`,`framework`.`UsersSystems`.`UserID` AS `UserID`,`framework`.`UsersSystems`.`SysCode` AS `SysCode`,`framework`.`UsersSystems`.`UserGroup` AS `UserGroup`,`framework`.`UsersSystems`.`EduGrpCode` AS `EduGrpCode`,`framework`.`UsersSystems`.`UserRole` AS `UserRole`,`framework`.`UsersSystems`.`UserR8s` AS `UserR8s`,`framework`.`UsersSystems`.`AccessType` AS `AccessType`,`framework`.`UsersSystems`.`UserSystemStatus` AS `UserSystemStatus`,`framework`.`UsersSystems`.`LogID` AS `LogID` from `framework`.`UsersSystems`;
-- --------------------------------------------------------
CREATE TABLE  `variables` (
  `varid` int(11) NOT NULL auto_increment,
  `var_name` varchar(100) collate utf8_persian_ci default NULL,
  `var_value` text collate utf8_persian_ci,
  `param1` varchar(500) collate utf8_persian_ci default NULL,
  `param2` varchar(500) collate utf8_persian_ci default NULL,
  PRIMARY KEY  (`varid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_persian_ci;

insert into variables values(1, 'post_id1', '6350056000', 'int', '');
...
insert into banks select * from hrms.banks;
-- --------------------------------------------------------
...
insert into countries select * from hrms.countries;
-- --------------------------------------------------------
CREATE TABLE  `states` (
...
insert into states select * from hrms.states;
-- --------------------------------------------------------
CREATE TABLE `cities` (
...
insert into cities select * from hrms.cities;
-- --------------------------------------------------------
...
insert into cost_centers select * from hrms_sherkati.cost_centers;
insert into cost_centers(title,cost_center_id) select c.title,c.cost_center_id from hrms.cost_centers as c on duplicate key UPDATE title=c.title;
...
insert into job_category select * from hrms.job_category;
-- --------------------------------------------------------
...
insert into job_subcategory select * from hrms.job_subcategory;
-- --------------------------------------------------------
...
insert into job_fields select * from hrms.job_fields;
...
insert into org_new_units select * from hrms.org_new_units;
-- --------------------------------------------------------
...
insert into persons select * from hrms.persons;
-- --------------------------------------------------------
CREATE TABLE  `staff` (
...
tafsili_id,UnitCode,FacCode,EduGrpCode,ProCode,DutyUnit,extra_work_coef,sum_paied_pension,retired_date,retired_state,job_id,post_id,ouid,NULL from hrms.staff;
...
s.job_id,s.post_id,s.ouid,s.person_type from hrms_sherkati.staff as s join hrms.persons as p using(PersonID);

alter table staff
Add Column base_work_start_date date default NULL COMMENT 'تاریخ شروع به کار با احتساب سنوات غیر رسمی'after ouid
...
insert into staff_groups select * from hrms.staff_groups;
insert into staff_groups select * from hrms_sherkati.staff_groups;
...
insert into position select * from hrms.position;
...
p.parent_path,p.is_dummy_post,p.staff_id,5,'0000-00-00',0 from hrms_sherkati.position as p join job_fields using(jfid) ;
-- --------------------------------------------------------
...
insert into person_dependents select pd.* from hrms.person_dependents as pd join hrms.persons using(PersonID);
-- --------------------------------------------------------
...
insert into person_dependent_supports select * from hrms.person_dependent_supports;
-- --------------------------------------------------------
...
insert into person_devotions select * from hrms.person_devotions;
-- --------------------------------------------------------
...
insert into person_employments select * from hrms.person_employments;
...
insert into study_fields select * from hrms.study_fields;
-- --------------------------------------------------------
...
insert into study_branchs select * from hrms.study_branchs;
-- --------------------------------------------------------
...
insert into universities select * from hrms.universities;
insert into universities select * from hrms_sherkati.universities on duplicate key
	update universities.country_id=hrms_sherkati.universities.country_id;
...
insert into person_educations select * from hrms.person_educations;

insert into study_branchs
	select pe.sfid,pe.sbid,'-','-' from hrms_sherkati.person_educations pe left join study_branchs s on(s.sfid=pe.sfid and s.sbid=pe.sbid)
...
insert into person_educations
	select pe.* from hrms_sherkati.person_educations as pe join hrms.persons p using(PersonID)
	left join person_educations hpe on(hpe.PersonID=pe.PersonID and pe.row_no=hpe.row_no)
	where hpe.PersonID is null;
	
ALTER TABLE `person_educations` ADD COLUMN `comments` TEXT AFTER `certificated`;
-- --------------------------------------------------------
...
insert into person_misc_docs select *,'' from hrms.person_misc_docs;
insert into person_misc_docs select *,'' from hrms_sherkati.person_misc_docs;
-- --------------------------------------------------------
...
insert into writ_types select * from hrms.writ_types;
...
insert into writ_subtypes select * from hrms.writ_subtypes;
...
insert into salary_item_types select * from hrms.salary_item_types;
...
insert into jobs select * from hrms.jobs;
insert into jobs select * from hrms_sherkati.jobs on duplicate key update jobs.job_id=hrms_sherkati.jobs.job_id;
...
insert into writs select * from hrms.writs;
...
insert into writs select w.writ_id,w.writ_ver,w.corrective,w.corrective_writ_id,w.corrective_writ_ver,w.corrective_date,w.staff_id,
  ...
  w.hortative_group,w.ouid_old,w.sub_ouid_old,w.related_onduty_year,w.related_onduty_month,w.related_onduty_day,w.writ_transfer_date,w.writ_recieve_date
   from hrms_sherkati.writs as w
	left join study_fields as f on(w.sfid=f.sfid)
  left join study_branchs as b on(w.sfid=b.sfid and w.sbid=b.sbid)
  left join position as p on(w.post_id=p.post_id);

ALTER TABLE `writs` ADD COLUMN `view_flag` TINYINT UNSIGNED DEFAULT 0 COMMENT 'تایید مشاهده حکم در پرتال' AFTER `writ_recieve_date`;
...
from hrms.writ_salary_items as s join hrms.writs as w on(s.writ_id=w.writ_id and s.writ_ver=w.writ_ver);

insert into writ_salary_items
select s.writ_id,s.writ_ver,w.staff_id,s.salary_item_type_id,s.param1,s.param2,s.param3,s.param4,s.value,s.automatic,
s.remember_date,s.remember_message,s.remembered,s.total_id,s.ledger_id,s.tafsili_id,s.tafsili2_id,s.must_pay,
s.base_value,s.edit_reason,s.param5,s.param6,s.param7
from hrms_sherkati.writ_salary_items as s join hrms_sherkati.writs as w on(s.writ_id=w.writ_id and s.writ_ver=w.writ_ver);

-- --------------------------------------------------------
CREATE TABLE  `salary_param_types` (
  `param_type` int(11) NOT NULL default '0',
  `title` varchar(45) collate utf8_persian_ci NOT NULL,
  `dim1_id` enum('STATE','CITY','SGROUP','EGROUP','SCIENCELEVEL','EDUCLEVEL','MARITALS','DUTY_YEAR') collate utf8_persian_ci default NULL,
  `dim2_id` enum('STATE','CITY','SGROUP','EGROUP','SCIENCELEVEL','EDUCLEVEL','MARITALS','DUTY_YEAR') collate utf8_persian_ci default NULL,
  `dim3_id` enum('STATE','CITY','SGROUP','EGROUP','SCIENCELEVEL','EDUCLEVEL','MARITALS','DUTY_YEAR') collate utf8_persian_ci default NULL,
  `dim4_id` enum('STATE','CITY','SGROUP','EGROUP','SCIENCELEVEL','EDUCLEVEL','MARITALS','DUTY_YEAR') collate utf8_persian_ci default NULL,
  `person_type` smallint(6) NOT NULL,
  PRIMARY KEY  USING BTREE (`param_type`,`person_type`)
...
,value,used from hrms.salary_params;

insert into salary_params(person_type,param_type,from_date,to_date,dim1_id,dim2_id,dim3_id
,value,used) select 5,param_type,from_date,to_date,dim1_id,dim2_id,dim3_id
,value,used from hrms_sherkati.salary_params;
...
;
insert into evaluation_lists select * from hrms.evaluation_lists;
insert into evaluation_list_items select * from hrms.evaluation_list_items;

insert into evaluation_lists select list_id+500,list_date,if(o.ouid is null,1,e.ouid) ,doc_state
from hrms_sherkati.evaluation_lists e left join org_new_units o using(ouid);
...
insert into evaluation_list_items select list_id+500,staff_id,functional_score,job_behaviour_score,social_behaviour_score
	,annual_coef,high_job_coef,comments,scores_sum from hrms_sherkati.evaluation_list_items;
...
insert into mobilization_lists select * from hrms.mobilization_lists;
insert into mobilization_list_items select * from hrms.mobilization_list_items;
-- ------------------------------------------------------------
...
insert into professor_exe_posts select * from hrms.professor_exe_posts;
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_persian_ci COMMENT='اطلاعات ممیزی ';

-- -----------------------------------------------------------

create or replace view payments as
  select * from hrms.payments
  union ALL
  select * from hrms_sherkati.payments;

create or replace view payment_items as
  select * from hrms.payment_items
  union ALL
  select row_id + (select count(*) from hrms.payment_items) + 1 AS row_id,pay_year AS pay_year,pay_month AS pay_month,staff_id AS staff_id,salary_item_type_id AS salary_item_type_id,pay_value AS pay_value,get_value AS get_value,param1 AS param1,param2 AS param2,param3 AS param3,param4 AS param4,param5 AS param5,param6 AS param6,cost_center_id AS cost_center_id,payment_type AS payment_type,debt_total_id AS debt_total_id,debt_ledger_id AS debt_ledger_id,debt_tafsili_id AS debt_tafsili_id,debt_tafsili2_id AS debt_tafsili2_id,cred_total_id AS cred_total_id,cred_ledger_id AS cred_ledger_id,cred_tafsili_id AS cred_tafsili_id,cred_tafsili2_id AS cred_tafsili2_id,diff_get_value AS diff_get_value,diff_pay_value AS diff_pay_value,diff_param1 AS diff_param1,diff_param2 AS diff_param2,diff_param3 AS diff_param3,diff_param4 AS diff_param4,diff_param5 AS diff_param5,diff_param6 AS diff_param6,diff_value_coef AS diff_value_coef,param7 AS param7,diff_param7 AS diff_param7,diff_param1_coef AS diff_param1_coef,diff_param2_coef AS diff_param2_coef,diff_param3_coef AS diff_param3_coef,diff_param4_coef AS diff_param4_coef,diff_param5_coef AS diff_param5_coef,diff_param6_coef AS diff_param6_coef,diff_param7_coef AS diff_param7_coef,param8 AS param8,param9 AS param9,diff_param8 AS diff_param8,diff_param9 AS diff_param9,diff_param8_coef AS diff_param8_coef,diff_param9_coef AS diff_param9_coef  from hrms_sherkati.payment_items;

create or replace view payment_writs as
  select * from hrms.payment_writs
  union ALL
  select pay_writ_id + (select count(*) from hrms.payment_writs) + 1 AS pay_writ_id,writ_id AS writ_id,writ_ver AS writ_ver,staff_id AS staff_id,pay_year AS pay_year,pay_month AS pay_month,payment_type AS payment_type from hrms_sherkati.payment_writs;

create or replace view payment_runs as
  select * from hrms.payment_runs
  union ALL
  select * from hrms_sherkati.payment_runs;

create or replace view writ_transcripts as
  select * from hrms.writ_transcripts 
  union ALL
  select transcript_id,if(person_type=6,5,person_type) as person_type,emp_state,title from hrms_sherkati.writ_transcripts;
-- -----------------------------------------------------------
...
---------------------------------------------------------------
 ...
 -- -------------------------------------------------------------
 CREATE TABLE  `writ_transcripts` (
  `transcript_id` smallint(6) NOT NULL auto_increment,
  `person_type` smallint(1) NOT NULL default '0',
  `emp_state` smallint(2) NOT NULL default '0',
  `title` varchar(250) collate utf8_persian_ci NOT NULL default '',
  PRIMARY KEY  (`transcript_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_persian_ci;

insert into writ_transcripts select * from hrms.writ_transcripts;
insert into writ_transcripts select * from hrms_sherkati.writ_transcripts;
update writ_transcripts set person_type=5 where person_type=6;
...
ALTER TABLE `pay_get_lists` ADD CONSTRAINT `FK_pay_get_lists_1` FOREIGN KEY `FK_pay_get_lists_1` (`cost_center_id`)
...
insert into pay_get_lists select * from hrms.pay_get_lists where list_id<>0;
insert into pay_get_lists select list_id+414300,list_date,salary_item_type_id,doc_state,list_type,total_id,ledger_id,tafsili_id,tafsili2_id,cost_center_id
	from hrms_sherkati.pay_get_lists;
-- ------------------------------------------------------------
...
insert into pay_get_list_items select * from hrms.pay_get_list_items;

insert into pay_get_list_items select p.list_id+414300,p.list_row_no,p.staff_id,p.salary_item_type_id,p.initial_amount,
p.approved_amount,p.value,p.comments,p.total_id,p.ledger_id,p.tafsili_id,p.tafsili2_id
from hrms_sherkati.pay_get_list_items p
join hrmstotal2.salary_item_types s using(salary_item_type_id);
